...
**Backend**: Express.js (CT 100, port 3500)
**Database**: Supabase (fenhyfxbapybmddvhcei)
**Package Manager**: npm
**Dev Command**: `npx expo start`
**Build Command**: `eas build`

## Skills Registry

```
building-ui        ~/.claude/skills/expo-app-design/skills/building-ui     Expo Router, controls, animations
tailwind-setup     ~/.claude/skills/expo-app-design/skills/tailwind-setup  Tailwind v4 + NativeWind v5
data-fetching      ~/.claude/skills/expo-app-design/skills/data-fetching   React Query, offline, caching
api-routes         ~/.claude/skills/expo-app-design/skills/api-routes      Server-side API routes
dev-client         ~/.claude/skills/expo-app-design/skills/dev-client      TestFlight, custom builds
use-dom            ~/.claude/skills/expo-app-design/skills/use-dom         Web in native (charts)
deployment         ~/.claude/skills/expo-deployment/skills/deployment       App Store, Play Store
cicd-workflows     ~/.claude/skills/expo-deployment/skills/cicd-workflows  EAS workflow YAML
vercel-ai-testing  ~/.claude/skills/vercel-ai-testing                      AI SDK v6, model testing
ios-native         ~/.claude/skills/ios-native                             Swift, Dynamic Island
react-best-prax    ~/.claude/skills/react-best-practices                   Patterns, performance
```

## Critical Paths

- `app/(tabs)/` — Tab navigation (Home, Chat, Health, Settings)
- `server/routes/chat.ts` — AI streaming endpoint
- `lib/ai/claude-code.ts` — Claude Code provider
- `lib/ai/gateway.ts` — AI Gateway models
- `lib/health/healthkit.ts` — HealthKit integration
- `departments/` — Agent context modules
...
**Docs**: https://ai-sdk.dev/providers/community-providers/claude-code

```
Key APIs:
- createClaudeCode({ defaultSettings }) → provider factory
- claudeCode('sonnet') | claudeCode('opus') | claudeCode('haiku') → model shortcuts
- permissionMode: 'bypassPermissions' | 'default' | 'acceptEdits'
- cwd, systemPrompt, maxTurns, settingSources, mcpServers
- Uses Claude's built-in tools (Bash, Read, Write, Edit) — NOT custom Zod tools
- Requires Claude Code CLI installed + authenticated (claude login)
```

## Models (March 2026)

| Provider | Model | ID | Use |
|----------|-------|----|-----|
| Claude Code | Sonnet 4.6 | `sonnet` | Primary — full agent w/ tools |
| Anthropic | Sonnet 4.6 | `claude-sonnet-4-6` | Fast chat, no tools |
| OpenAI | GPT 5.3 | `gpt-5.3` | Alt perspective |
| Google | Gemini 3.1 | `gemini-3.1` | Multimodal |
